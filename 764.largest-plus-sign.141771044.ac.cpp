        ...
        for(int i=0;i<N;i++)
            for(int j=0;j<N;j++)
                if(j==0){
                    ...
                    left[i][j]=(dp[i][j])?(left[i][j-1]+dp[i][j]):0;
                    right[i][N-1-j]=(dp[i][N-1-j])?right[i][N-j]+dp[i][N-1-j]:0;
                }
        for(int i=0;i<N;i++)
            for(int j=0;j<N;j++)
                if(j==0){
                    ...
                    top[j][i]=(dp[j][i])?(top[j-1][i]+dp[j][i]):0;
                    bottom[N-1-j][i]=(dp[N-1-j][i])?bottom[N-j][i]+dp[N-1-j][i]:0;
                }
        int max_value=0;
        for(int i=0;i<N;i++)
            for(int j=0;j<N;j++)
                max_value=max(max_value,min(min(top[i][j],bottom[i][j]),min(right[i][j],left[i][j])));
        return max_value;
    }
};
