...
public:
    vector<int> findOrder(int numCourses, vector<pair<int, int>>& prerequisites) {
        for(int i=0;i<prerequisites.size();i++)
            mp[prerequisites[i].second].insert(prerequisites[i].first);
        vector<bool> visited(numCourses,false);
        vector<bool> onpath(numCourses,false);
        vector<int> top;
        for(int i=0;i<numCourses;i++)
            if(!visited[i] && !dfs(visited,i,onpath,top))
                return {};
        std::reverse(top.begin(),top.end());
        return top;
    }
    bool dfs(vector<bool> &visited,int pos,vector<bool> &onpath,vector<int> &top){
        if(onpath[pos]) return false;
        if(visited[pos]) return true;
        onpath[pos]=visited[pos]=true;
        for(auto i:mp[pos])
            if(onpath[i] || !dfs(visited,i,onpath,top))
                return false;
        top.push_back(pos);
        onpath[pos]=false;
        return true;
