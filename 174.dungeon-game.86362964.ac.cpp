...
public:
    int calculateMinimumHP(vector<vector<int>>& dun) {
        ...
        res[row-1][col-1]=max(1-dun[row-1][col-1],1);
        for(int i=row-2;i>=0;i--)
        {
            res[i][col-1]=max(res[i+1][col-1]-dun[i][col-1],1);
        }
        for(int i=col-2;i>=0;i--)
        {
            res[row-1][i]=max(res[row-1][i+1]-dun[row-1][i],1);
        }
        for(int i=row-2;i>=0;i--)
        {
            for(int j=col-2;j>=0;j--)
            {
                res[i][j]=max(min(res[i+1][j],res[i][j+1])-dun[i][j],1);
            }
        }
        return res[0][0];
    }
