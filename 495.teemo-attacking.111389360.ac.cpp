...
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        if(!timeSeries.size() || !duration)
        {
            return 0;
        }
        int pos=0;
        //this is the initial starting pos
        int reach=0;
        //this is the poisoned duration
        int res=0;
        while(pos<timeSeries.size())
        {
            int initial_reach=timeSeries[pos];
            //this is the intial reach before changing
            //std::cout<<initial_reach<<"\n";
            reach=initial_reach+duration;
            while(pos<timeSeries.size() && timeSeries[pos]<reach )
            {
                reach=timeSeries[pos]+duration;
                //std::cout<<reach<<"\n";
                pos++;
            }
            //pos++;
            //std::cout<<reach<<"\n";
            res+=(reach-initial_reach);
        }
        return res;
    }
};
